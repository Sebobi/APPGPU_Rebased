...
__global__ void mover_withoutSUB(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param){
...
int mover_PC_GPU(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param, int size)
{
    ...
    for (int i_sub=0; i_sub <  part->n_sub_cycles; i_sub++){
        ...
        mover_withoutSUB<<<blocks,threadsPerBlock>>>(gpu_parts, field, grd, param);
